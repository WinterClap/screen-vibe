...
export const usePagination = ({ totalPages, siblingCount, currentPage, buttonSpreadThreshold }: HookArgs) => {
  const paginationRange = React.useMemo(() => {
    // Pages count is determined as siblingCount + firstPage + lastPage + currentPage + 2*DOTS
    const totalPageNumbers = siblingCount + buttonSpreadThreshold;
    ...
    return [];
  }, [totalPages, siblingCount, currentPage, buttonSpreadThreshold]);
