...
export type AccountMoviesFavoritesDetails = {
  page: number;
  results: {
...
export type AccountTvFavoritesDetails = {
  page: number;
  results: {
    poster_path: string | null;
    overview: string;
...
export type AccountFavoritesData =
  | {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<AccountFavoritesData>) {
  const { method } = req;
  ...
  switch (method) {
    case "GET": {
      ...
      if (mediaType === "movies") {
        try {
          const tmbdResponse = await fetch(
            `${tmbd_base_url}/account/${account_id}/favorite/movies?` +
              new URLSearchParams({
                api_key,
                session_id,
                page: page.toString(),
      ...
      try {
        const tmbdResponse = await fetch(
          `${tmbd_base_url}/account/${account_id}/favorite/tv?` +
            new URLSearchParams({
              api_key,
              session_id,
              page: page.toString(),
    ...
    case "POST": {
      ...
      try {
        const tmdbResponse = await fetch(
          `${tmbd_base_url}/account/${account_id}/favorite?` +
            new URLSearchParams({
              api_key,
              session_id,
            }),
